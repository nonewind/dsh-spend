...
usageEndpoints;
    ...
    async fetchProviderUsage(provider) {
      const adapter = PROVIDER_USAGE[provider];
      ...
      try {
        const usage = adapter !== undefined
          ...
          : await this.fetchGenericUsage(provider, endpoint);
        data = { ...usage, provider, source: "provider", fetchedAt: Date.now(), error: null };
      } catch (error) {
        data = {
          provider,
          source: "provider",
    ...
    async fetchProviderBalance(provider) {
      const adapter = PROVIDER_BALANCE[provider];
      if (adapter === undefined) return undefined;
      const endpoint = this.usageEndpoints.find((entry) => entry.provider === provider);
      const cacheKey = `${provider}\u0000balance`;
      const cached = this.usageCache.get(cacheKey);
      if (cached !== undefined && Date.now() - cached.at < this.usageTtlMs) return cached.data;
      let data;
      try {
        const balance = await adapter.fetch(this.providerIo(), endpoint ?? null);
        data = { provider, source: "provider", fetchedAt: Date.now(), balance, error: null };
      } catch (error) {
        data = {
          provider,
          source: "provider",
    ...
    async compute(signature, live, cwd) {
      ...
      const balanceSeen = new Set(mergedPlans.map((plan) => plan.provider));
