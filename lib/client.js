...
`;
		...
		function Heatmap({ days, t, currency }) {
			...
			cells.forEach((cell, index) => {
				...
				if (month !== lastMonth) {
					lastMonth = month;
					monthTicks.push({ x: (index / 7) * step + 3, label: String(month) });
				}
			});
							...
							react_jsx_runtime.jsx("svg", {
								...
								children: [
									...
									cells.map((cell, index) => {
										...
										const row = index % 7;
										return react_jsx_runtime.jsx("rect", { x: col * step, y: HEAT_TOP + row * step, width: HEAT_CELL, height: HEAT_CELL, rx: 2, fill: HEAT_COLORS[levelOf(cell.tokens)] }, cell.key);
									}),
								],
							}),
						],
