...
.dsu-chartX{display:flex;justify-content:space-between;color:var(--dsw-alias-label-tertiary,#8c959f);font-size:10px;line-height:16px;margin-top:2px;font-variant-numeric:tabular-nums}
...
`;
		...
		function buildQuery(ctx) {
			return async () => {
				const result = await ctx.connection.rpc.call("/api", "usageStats/query", { args: {} });
		 ...
		 * fees for code plans plus estimated cost for token plans. */
		function SummaryCards({ t, totals, callCount, sessionsScanned, currency, billing }) {
			...
			return react_jsx_runtime.jsx("div", {
				...
				children: [
					...
					react_jsx_runtime.jsx(Card, { label: t("summary.input"), value: formatTokens(totals.inputTokens), title: formatTokensFull(totals.inputTokens) }),
					react_jsx_runtime.jsx(Card, { label: t("summary.output"), value: formatTokens(totals.outputTokens), title: formatTokensFull(totals.outputTokens) }),
					react_jsx_runtime.jsx(Card, { label: t("summary.cacheRead"), value: formatTokens(totals.cacheReadTokens), title: formatTokensFull(totals.cacheReadTokens) }),
					react_jsx_runtime.jsx(Card, { label: t("summary.cacheWrite"), value: formatTokens(totals.cacheWriteTokens), title: formatTokensFull(totals.cacheWriteTokens) }),
		...
		const CHART_W = 640;
		const CHART_H = 180;
		const CHART_PAD = { top: 12, right: 10, bottom: 6, left: 10 };
		const CURVE_COLORS = {
			inputTokens: "#4f8ef7",
			outputTokens: "#2da44e",
		...
		function hourLabel(hour) {
			// "YYYY-MM-DD HH:00" → "MM-DD HH:00" (full date at midnight)
			const [, month, day, hh] = hour.match(/^(\d{4})-(\d{2})-(\d{2}) (\d{2}):00$/) ?? [];
			if (hh === undefined) return hour;
			return hh === "00" ? `${month}-${day}` : `${hh}:00`;
		}
		function TimeCurveChart({ hours, currency, t }) {
			...
			const maxValue = Math.max(1, ...series.flatMap((s) => hours.map((h) => Number(h[s.key]) || 0)));
			const xAt = (index) => CHART_PAD.left + (hours.length <= 1 ? plotW / 2 : (index / (hours.length - 1)) * plotW);
			const yAt = (value) => CHART_PAD.top + plotH - (Number(value) / maxValue) * plotH;
			const linePath = (s) => hours
				.map((h, i) => `${i === 0 ? "M" : "L"}${xAt(i).toFixed(1)},${yAt(h[s.key] ?? 0).toFixed(1)}`)
				.join(" ");
			const areaPath = (s) => `${linePath(s)} L${xAt(hours.length - 1).toFixed(1)},${(CHART_PAD.top + plotH).toFixed(1)} L${xAt(0).toFixed(1)},${(CHART_PAD.top + plotH).toFixed(1)} Z`;
			const gridLines = [0.25, 0.5, 0.75, 1].map((fraction) => ({
				y: CHART_PAD.top + plotH - fraction * plotH,
				label: formatTokens(maxValue * fraction),
			}));
			const tickEvery = Math.max(1, Math.ceil(hours.length / 6));
			const ticks = hours.map((h, i) => ({ i, label: hourLabel(h.hour) })).filter((_, i) => i % tickEvery === 0 || i === hours.length - 1);

			const onMove = (event) => {
				const rect = event.currentTarget.getBoundingClientRect();
				if (rect.width === 0) return;
				const x = ((event.clientX - rect.left) / rect.width) * CHART_W;
				const index = Math.round(((x - CHART_PAD.left) / plotW) * (hours.length - 1));
				setHover(Math.max(0, Math.min(hours.length - 1, index)));
			};
			...
			return react_jsx_runtime.jsx("div", {
				...
				children: [
					...
					react_jsx_runtime.jsx("svg", {
						...
						children: [
							gridLines.map((line) => react_jsx_runtime.jsx("line", { x1: CHART_PAD.left, x2: CHART_W - CHART_PAD.right, y1: line.y, y2: line.y, stroke: "var(--dsw-alias-border-l2,rgba(128,128,128,.14))", strokeWidth: 1 }, line.label)),
							mode === "cost"
							...
							hover !== null
								? [
									...
									series.map((s) => react_jsx_runtime.jsx("circle", { cx: hoverX, cy: yAt(hours[hover][s.key] ?? 0), r: 3, fill: s.color, stroke: "var(--dsw-alias-bg-base,#ffffff)", strokeWidth: 1.5 }, `dot-${s.key}`)),
								]
								: null,
						],
					}),
					react_jsx_runtime.jsx("div", {
						...
						children: ticks.map((tick) => react_jsx_runtime.jsx("span", { children: tick.label }, tick.i)),
					}),
				],
			});
		}
		...
		function PerfChart({ hours, t }) {
			...
			const active = rows.filter((row) => typeof valueOf(row) === "number" && (row.samples ?? 0) > 0);
			if (active.length < 2) {
			...
			const plotW = CHART_W - CHART_PAD.left - CHART_PAD.right;
			const plotH = CHART_H - CHART_PAD.top - CHART_PAD.bottom;
			const xAt = (index) => CHART_PAD.left + (rows.length <= 1 ? plotW / 2 : (index / (rows.length - 1)) * plotW);
			const yAt = (value) => CHART_PAD.top + plotH - (Number(value) / maxValue) * plotH;
			...
			const pathOf = (segment) => segment
				.map((row, i) => `${i === 0 ? "M" : "L"}${xAt(rows.indexOf(row)).toFixed(1)},${yAt(valueOf(row)).toFixed(1)}`)
				.join(" ");
			const gridLines = [0.25, 0.5, 0.75, 1].map((fraction) => ({
				y: CHART_PAD.top + plotH - fraction * plotH,
			...
			const ticks = rows.map((row, i) => ({ i, label: hourLabel(row.hour) })).filter((_, i) => i % tickEvery === 0 || i === rows.length - 1);
			const onMove = (event) => {
				const rect = event.currentTarget.getBoundingClientRect();
				if (rect.width === 0) return;
				const x = ((event.clientX - rect.left) / rect.width) * CHART_W;
				const index = Math.round(((x - CHART_PAD.left) / plotW) * (rows.length - 1));
			...
			const tooltipText = hovered === null || hoverValue === null
				? ""
				: `${hourLabel(hovered.hour)} · ${mode === "ttft" ? formatMs(hoverValue) : formatTps(hoverValue)} · ${t("perf.samples")} ${hovered.samples}`;
		...
		function TodaySection({ t, data, currency }) {
			...
			const todayHours = (data.byHour ?? []).filter((row) => row.hour.startsWith(todayKey));
...
.meta{color:#8c959f;font-size:12px;margin-bottom:16px}
...
.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}
		...
		function Dashboard({ t, data, error, lastUpdated, onRefresh, onOpenDetails }) {
			...
			const overview = react_jsx_runtime.jsx(react_jsx_runtime.Fragment, {
				children: [
					react_jsx_runtime.jsx(SummaryCards, { t, totals, callCount: data.callCount, sessionsScanned: data.sessionsScanned, currency, billing: data.billing }),
			...
			const details = react_jsx_runtime.jsx(Section, {
			...
			return react_jsx_runtime.jsx("div", {
				...
				children: [
						...
						? react_jsx_runtime.jsx("div", {
							...
							children: [
								...
								react_jsx_runtime.jsx("div", {
									...
									children: react_jsx_runtime.jsx(Dashboard, { t, data, error, loading, lastUpdated, onRefresh: refresh, onOpenDetails: openDetails }),
								}),
							],
						})
						: null,
					react_jsx_runtime.jsx("button", {
						...
						children: [
							react_jsx_runtime.jsx("span", { className: "dsu-pillDot", style: error !== null ? { background: "var(--dsw-alias-state-warning-primary,#bf8700)" } : undefined }),
