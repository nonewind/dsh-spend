...
- **点击**：展开详情面板，四个标签页：
  ...
  - **总览**（仪表盘，参考主流用量面板的 KPI + 趋势布局）：**统计栏**（预计花费（月）+ 构成、按 token 估算、总 Token、调用、会话）+ **计划用量**（自动识别 Code/Token 计划、档位、额度使用与剩余）+ 72 小时时间曲线 + **费用 Top 提供商 / Top 模型**（各 6 行）+ 近 31 天趋势；
  - **今日**：当天的调用数、Token 与费用小结 + **今日逐小时**的 Token / 费用图表；
  - **性能**：每个模型的**首字延时（TTFT）均值 / P50 / P90、生成速度（tokens/s）、总延迟均值**，以及按小时的 TTFT / 速度曲线；
  - **调用明细**：**每个会话 × 模型**的调用次数、token 与费用明细 + **按会话统计** + **最近调用** + **计费单价表**，可在**独立窗口**中打开（随主窗口自动刷新）。
